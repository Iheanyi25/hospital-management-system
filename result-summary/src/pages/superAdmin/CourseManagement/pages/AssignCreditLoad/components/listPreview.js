...
export const ListPreview = ({
	...
	const uploadList = () => {
		...
		formData.append("sessionId", getValues().sessionId?.value);
		formData.append("semesterId", getValues().semesterId?.value);
		const requestBody = {
			url: uploadBulkStudentUnitLoadUrl(),
			data: formData
		};
		mutate(requestBody, {
			onSuccess: () => {
			...
			>
				<div className="container-fluid px-4 mt-3 mb-3">
					<p>
						...
						unit loads, before you click complete upload
					</p>
					<div>
						{extractedData?.rows ? (
							<DisplyTable
