...
import {
	deactivateApplicationSundryInvoiceUrl,
...
const DeleteSundryInvoice = () => {
	...
	let details = [
		...
		{ title: "Invoice Number", value: data?.data?.invoiceCode },
		{ title: "Amount", value: data?.data?.breakDown?.[0]?.amount },
		{
			title: "Invoice Description",
			value: data?.data?.breakDown?.[0]?.description
		},
		{
	...
	const onSubmit = () => {
		const requestBody = {
			url: deactivateApplicationSundryInvoiceUrl(data?.data?.invoiceCode)
		};
...
export default DeleteSundryInvoice;
