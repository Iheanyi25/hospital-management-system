...
import { downloadFacultyExcelTemplateUrl } from "../../../../../../api/urls";
...
export const FacultyUpload = ({ setUploaded, setFileData }) => {
	...
	const {
		...
		error: fileError
	} = useApiBlob(downloadFacultyExcelTemplateUrl(), {
	...
	const downloadXLSFile = useCallback(async () => {
		...
		if (fileError || !file?.data) {
			...
			const outputFilename = `Bulk Faculties Upload Template.xlsx`;
			// file file actions.
