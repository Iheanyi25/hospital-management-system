...
import { downloadOptionExcelTemplateUrl } from "../../../../../../api/urls";
...
export const OptionstUpload = ({ setUploaded, setFileData }) => {
	...
	const {
		...
		error: fileError
	} = useApiBlob(downloadOptionExcelTemplateUrl(), {
	...
	const downloadXLSFile = useCallback(async () => {
		...
		if (fileError || !file?.data) {
			...
			const outputFilename = `Bulk Options Upload Template.xlsx`;
			// file file actions.
