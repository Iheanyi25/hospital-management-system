...
const Default = () => {
	...
	const { data, isLoading, error } = useApiGet(
		getMyInvoicesUrl(PAYMENTIDENTIFIER.sundry)
	);
	...
	useEffect(() => {
		if (invoiceData?.success && makeRequest && !isLoading) {
			push({
				pathname: "/academic_fees/sundry/invoice",
	...
	const generateBalanceInvoice = useCallback(
		(values) => {
			...
			const data = {
				...
				paymentPurpose: values?.paymentPurposeId
			};
			const requestBody = {
		...
		firstTime: {
			...
			buttonGroup: (
				<Button
					...
					buttonClass="primary"
					onClick={() => push("/academic_fees/sundry/session")}
				/>
			)
		}
	};
						...
						{row.original.hasBalanceToPay ? (
							...
							<Button
								...
								loading={
									...
									row.original.rrr === rrr
								}
								onClick={() => onSubmit(row.original.rrr)}
							/>
						)}
						<Button
							...
							onClick={() =>
								push({
									pathname: "/academic_fees/sundry/receipt",
									state: {
					...
					<>
						{data?.data.length !== 0 && (
							<Button
								...
								buttonClass="primary"
								onClick={() =>
									push("/academic_fees/sundry/session")
								}
							/>
						)}
					</>
