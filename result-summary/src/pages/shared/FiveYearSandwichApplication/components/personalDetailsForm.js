...
import { storeFiveYearSandwichApplicationPersonalDetailsUrl } from "../../../../api/urls";
...
export const PersonalDetailsForm = ({
	...
	const { basicInformation } = fiveYearSandwichState;
	...
	const onSubmit = (values) => {
		if (!fiveYearSandwichState?.programme?.passport) {
			window.scrollTo(0, 0);
		...
		const basicInfo = {
			...
			passportAsBase64: fiveYearSandwichState?.programme?.passport,
		...
		const requestBody = {
			url: storeFiveYearSandwichApplicationPersonalDetailsUrl(),
			data
		};
		mutate(requestBody, {
			onSuccess: (data) => {
