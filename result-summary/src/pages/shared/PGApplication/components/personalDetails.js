...
import { personalDetailsSchema } from "../pgSchema";
...
export const PersonalDetails = ({
	...
	const { basicInformation, nextOfKin } = useSelector(
		(state) => state.pgData
	);
	...
	const {
		...
		defaultValues: {
			...
			otherNames: basicInformation?.otherNames,
			sexId: basicInformation?.genderId,
			...
			maritalStatusId: basicInformation?.maritalStatusId,
			fullname: nextOfKin?.fullname,
			address: nextOfKin?.address,
			phoneNo: nextOfKin?.mobileNumber,
			relationshipId: nextOfKin?.relationshipId
		},
