...
const CCEApplication = () => {
	const cce = useSelector((state) => state.cceData);
	const { passport, personalInfoResponse } = cce;
	...
	const { data: departments, isLoading: isLoadingDepartments } = useApiGet(
		getDepartmentsUrl(cce?.StudentTypeId?.value),
		{
...
const DisplayInformation = memo(
	({
		...
		switch (location.hash) {
			...
			case "#section_b":
				return <ProgrammeDetails allDepartments={allDepartments} />;
			case "#section_c":
				return (
					<OlevelResult
