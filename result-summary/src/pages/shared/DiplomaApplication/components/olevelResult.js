...
import { OlevelResultSchema } from "../diplomaSchema";

import { Bin, ChevronDownFilled, RedCancel } from "../../../../assets/svgs";
import { setDiplomaApplicationOlevelDetails } from "../../../../api/urls";
...
export const OlevelResult = ({
	...
	const {
		...
		defaultValues: {
			sittings: diplomaState?.oLevelResult?.sittings?.map((sitting) => ({
	...
	const onSubmit = (oLevelResult) => {
		const requestBody = {
			url: setDiplomaApplicationOlevelDetails(),
			data: {
		...
		mutate(requestBody, {
			onSuccess: () => {
				const successFlag = window.AJS.flag({
					...
					body: "Your O-Level results have been successfully updated."
				});
				setTimeout(() => {
					successFlag.close();
				}, 5000);
	...
	useEffect(() => {
		if (diplomaState?.oLevelResult?.sittings?.length === 2) {
			setSittings((prevSittings) => [
				...prevSittings,
				prevSittings[prevSittings?.length - 1] + 1
			]);
	...
	}, [diplomaState?.oLevelResult?.sittings?.length]);
				...
				<div key={sittingIndex}>
					<Jumbotron
						headerText={
							sittingIndex === 0 ? (
								<span>
									First Sitting <CompulsoryIndicator />
								</span>
							) : (
								<span>
									Second Sitting <CompulsoryIndicator />
								</span>
							)
						}
						endText={
