...
export default function PharmacistBio({ pharmDet }) {
	const { pharmacy: { firstName, lastName, phoneNumber, email, userType } } = pharmDet;
	return (
		<div>
			<header className="page-header ml-3">
				<h3 className="page-title">{`Pharm. ${firstName || ''} ${lastName || ''}`}</h3>
			</header>
