...
export default function EditInfo({ otherDetails, userId : userProfileId, primaryDetails, mutate, setSucces }) {
	...
	const handleSubmit = async (e) => {
		...
		const postBasicInfoUrl = fetchConfig({
			url: basicInfoUrl,
			method: 'post',
			data: { [id]:userId, gender, dateOfBirth, firstName, lastName, otherNames, age: otherDetails.age }
		});
	...
	return (
		<div className="modal fade" id="edit-info" tabIndex="-1" role="dialog" aria-hidden="true">
			<div className="modal-dialog modal-dialog-centered">
				<div className="modal-content">
					<div className="modal-body">
						<h5 className="text-center">Edit Contact information</h5>
						<ProfileInfoForm details={details} handleChange={handleChange} handleSubmit={handleSubmit} />
					</div>
				</div>
			</div>
		</div>
