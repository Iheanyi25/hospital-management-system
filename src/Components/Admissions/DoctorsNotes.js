...
class DoctorsNotes extends React.Component {
  state = {
    prescriptions: [],
    loading: true,
  };
  ...
  fetchDoctorsNotes = async () => {
    ...
    try {
      ...
      const getAdmissionsDoctorsNoteConfig = fetchConfig({
        url: getAdmissionsDoctorsNote,
        method: "GET",
      });
      const { data, status } = await fetchWrapper(
        getAdmissionsDoctorsNoteConfig
        );
      this.setState({
        prescriptions: data.clerkingHistory,
      });
      if (status === 200) {
      ...
      <div className="card-body">
        ...
        <div id="accordion" className="mb-3">
          {loading ? (
            ...
            prescriptions.map((clerkingHistory, index) => (
