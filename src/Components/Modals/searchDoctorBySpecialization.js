...
class SearchDoctorsBySpecializationModal extends React.Component {
  ...
  startSearching = async (e) => {
    ...
    const getDoctorsBySpecialization = getDoctorsBySpecializationUrl(
      e.target.value
    );
    const getDoctorsBySpecializationConfig = fetchConfig({
      url: getDoctorsBySpecialization,
      method: "get",
    });
    const { data } = await fetchWrapper(getDoctorsBySpecializationConfig);

    this.setState({ searching: false, doctors: data.doctors });
  };
                    ...
                    this.state.doctors.length > 0 ? (
                      ...
                      >
                        {this.state.doctors.map((item, index) => (
                          <li
                            className={`cursor-pointer searchStringValue ${
                              doctorId === item.doctorProfile?.doctorId
                                ? "active"
                                : ""
                            }`}
                            key={index}
                          ...
                          >
                            {item.doctorProfile?.doctor.lastName}{" "}
                            {item.doctorProfile?.doctor.firstName}
                          </li>
                        ))}
                      </ul>
                    ) : (
