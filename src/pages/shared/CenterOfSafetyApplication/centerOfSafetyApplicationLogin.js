...
const CenterOfSafetyApplicationLogin = () => {
	...
	const {
		...
		handleSubmit,
		formState: { errors }
	} = useForm({ resolver: yupResolver(CSPGApplicationsLoginSchema) });

	const {
		data,
		isFetching,
		error: requestError
	} = useApiGet(CSPGAppVerificationPaymentUrl(rrr), {
		enabled: makeRequest,
		...
		if (data?.success && makeRequest && !isFetching) {
			if (!data?.data?.isFormCompleted) {
				dispatch({
					...
					payload: CSPGApplicationsInitialState(data?.data)
				});
				push({
					pathname: "/center_of_safety_application",
		...
		<AuthPageWrapper>
			<form onSubmit={handleSubmit(onSubmit)} className="mt-4">
				...
				<p className={`${styles.auth_sub_header}`}>
					Login to fill Centre for Safety Education Application form
				</p>
...
export default CenterOfSafetyApplicationLogin;
