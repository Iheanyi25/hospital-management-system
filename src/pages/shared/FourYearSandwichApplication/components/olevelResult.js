...
import { OlevelResultSchema } from "../fourYearSandwichSchema";

import { Bin, ChevronDownFilled, RedCancel } from "../../../../assets/svgs";
import { storeFourYearSandwichApplicationOlevelDetailsUrl } from "../../../../api/urls";
...
export const OlevelResult = ({
	...
	const {
		...
		defaultValues: {
			sittings: fourYearSandwichState?.oLevelResult?.sittings?.map(
				(sitting) => ({
	...
	const onSubmit = (oLevelResult) => {
		const requestBody = {
			url: storeFourYearSandwichApplicationOlevelDetailsUrl(),
			data: {
				applicantId,
	...
	useEffect(() => {
		if (fourYearSandwichState?.oLevelResult?.sittings?.length === 2) {
			setSittings((prevSittings) => [
				...prevSittings,
				prevSittings[prevSittings?.length - 1] + 1
			]);
	...
	}, [fourYearSandwichState?.oLevelResult?.sittings?.length]);
