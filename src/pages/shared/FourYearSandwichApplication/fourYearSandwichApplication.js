...
const FourYearSandwichApplication = () => {
	const fourYearSandwichState = useSelector(
		(state) => state.fourYearSandwichData
	);
	const { programme, basicInformation } = fourYearSandwichState;
	...
	if (!state?.fromLogin) push("/four_year_sandwich_login");
	...
	const navs = useMemo(
		() => [
			...
			{
				linkName: "Sandwich Diploma Course",
				hashName: "#section_b",
				state
			},
			// {
...
export default FourYearSandwichApplication;
