...
import { OlevelResultSchema } from "../fiveYearSandwichSchema";

import { Bin, ChevronDownFilled, RedCancel } from "../../../../assets/svgs";
import { storeFiveYearSandwichApplicationOlevelDetailsUrl } from "../../../../api/urls";
...
export const OlevelResult = ({
	...
	const {
		...
		defaultValues: {
			sittings: fiveYearSandwichState?.oLevelResult?.sittings?.map(
				(sitting) => ({
	...
	const onSubmit = (oLevelResult) => {
		const requestBody = {
			url: storeFiveYearSandwichApplicationOlevelDetailsUrl(),
			data: {
				applicantId,
	...
	useEffect(() => {
		if (fiveYearSandwichState?.oLevelResult?.sittings?.length === 2) {
			setSittings((prevSittings) => [
				...prevSittings,
				prevSittings[prevSittings?.length - 1] + 1
			]);
	...
	}, [fiveYearSandwichState?.oLevelResult?.sittings?.length]);
