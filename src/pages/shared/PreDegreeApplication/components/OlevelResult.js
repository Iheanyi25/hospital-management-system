...
import { OlevelResultSchema } from "../predegreeSchema";

import { Bin, ChevronDownFilled, RedCancel } from "../../../../assets/svgs";
import { updatePredegreeOlevelFormUrl } from "../../../../api/urls";
...
export const OlevelResult = ({
	...
	const {  applicantId } = predegreeData;
	...
	const {
		...
		defaultValues: {
			sittings: predegreeData?.oLevelResult?.sittings?.map((sitting) => ({
	...
	const onSubmit = (oLevelResult) => {
		const requestBody = {
			url: updatePredegreeOlevelFormUrl(),
			data: {
				applicantId,
	...
	useEffect(() => {
		if (predegreeData?.oLevelResult?.sittings?.length === 2) {
			setSittings((prevSittings) => [
				...prevSittings,
				prevSittings[prevSittings?.length - 1] + 1
			]);
	...
	}, [predegreeData?.oLevelResult?.sittings?.length]);
