...
import { supplementaryPutmeInitialState } from "../../../store/reducers/supplementaryPutmeReducer";
import { useApiGet } from "../../../api/apiCall";
import AuthPageWrapper from "../AuthPageWrapper";

export const supplementaryPutmeLoginSchema = yup.object().shape({
	jambRegNumber: yup.string().required("please input your reg. number")
});

const SupplementaryPutmeLogin = () => {
	...
	const {
		...
		handleSubmit,
		formState: { errors }
	} = useForm({ resolver: yupResolver(supplementaryPutmeLoginSchema) });

	const {
		data,
		isFetching: isLoading,
		error: requestError
	} = useApiGet(supplementaryPutmeLoadApplicationFormUrl(jambRegNumber), {
		enabled: makeRequest,
	...
	useEffect(() => {
		if (data?.success && makeRequest && !isLoading) {
			dispatch({
				...
				payload: supplementaryPutmeInitialState(data?.data)
			});
			if (data?.data?.formCompleted === true) {
				...
				replace({
					pathname: "/supplementary_putme_application",
			...
			<AuthPageWrapper>
				<form onSubmit={handleSubmit(onSubmit)} className="mt-4">
					...
					<p className={`${styles.auth_sub_header}`}>
						SUPPLEMENTARY PUTME APPLICATION FORM
					</p>
...
export default SupplementaryPutmeLogin;
