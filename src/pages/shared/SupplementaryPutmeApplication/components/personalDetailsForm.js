...
import { postSupplementaryPutmePersonalDetailsFormUrl } from "../../../../api/urls";
...
export const PersonalDetailsForm = ({
	...
	const onSubmit = (personalInfoResponse) => {
		if (!supplementaryPutmeData?.passport?.passport) {
			window.scrollTo(0, 0);
		...
		const requestBody = {
			url: postSupplementaryPutmePersonalDetailsFormUrl(),
			data: {
				regNumber: supplementaryPutmeData?.JambRegNumber,
				...
				passportAsBase64: supplementaryPutmeData?.passport?.passport
			}
		};
		mutate(requestBody, {
			onSuccess: (data) => {
						...
						<div className="row">
							...
							<div className="col-lg-9">
								<Controller
									...
									control={control}
									defaultValue={
										supplementaryPutmeData?.StateId?.value
											? findValueAndLabel(
													supplementaryPutmeData
														?.StateId?.value,
													states
						...
						<div className="row">
							...
							<div className="col-lg-9">
								<Controller
									...
									control={control}
									defaultValue={
										supplementaryPutmeData?.LgaId?.value
											? findValueAndLabel(
													supplementaryPutmeData
														?.LgaId?.value,
													localGovernments
