...
import { OlevelResultSchema } from "../uniTransferSchema";

import { Bin, ChevronDownFilled, RedCancel } from "../../../../assets/svgs";
import { updateTransferApplicationOlevelDetailsUrl } from "../../../../api/urls";
...
export const OlevelResult = ({
	...
	const { applicantId } = uniTransferState;
	...
	const {
		...
		defaultValues: {
			sittings: uniTransferState?.oLevelResult?.sittings?.map(
				(sitting) => ({
	...
	const onSubmit = (oLevelResult) => {
		const requestBody = {
			url: updateTransferApplicationOlevelDetailsUrl(),
			data: {
				applicantId,
	...
	useEffect(() => {
		if (uniTransferState?.oLevelResult?.sittings?.length === 2) {
			setSittings((prevSittings) => [
				...prevSittings,
				prevSittings[prevSittings?.length - 1] + 1
			]);
	...
	}, [uniTransferState?.oLevelResult?.sittings?.length]);
