...
import { useGetObjectByLabel } from "../../../../../../custom-hooks/useGetObjectByLabel";
...
const SelectSession = () => {
	...
	const paymentPurpose = useGetObjectByLabel(
		allPaymentPurposes,
		"Change Of Department"
	);

	useEffect(() => {
		setValue("paymentPurposeId", paymentPurpose);
	...
	useEffect(() => {
		if (data?.success && makeRequest && !makingRequest) {
			history.push({
				pathname: "/academic_fees/change_of_degree/generate_invoice",
				state: data?.data
			});
		}
