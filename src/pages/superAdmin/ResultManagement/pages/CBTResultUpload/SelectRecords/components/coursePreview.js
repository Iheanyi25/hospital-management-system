...
export const CoursePreview = ({
	...
	const uploadCourses = () => {
		...
		formData.append("courseId", currentFilterState?.courseId);
		formData.append("semesterId", currentFilterState?.semesterId);
		formData.append("hasCA", currentFilterState?.hasCA);

		const requestBody = {
			url: uploadCBTScoresheetUrl(),
			data: formData
		};
		mutate(requestBody, {
			onSuccess: () => {
				queryClient.invalidateQueries(
					getCourseGradesUrl(currentFilterState)
				);
			...
			onError: (error) => {
				const errorFlag = window.AJS.flag({
					...
					body: error?.response?.data?.message || `an error occurred.`
				});
				setTimeout(() => {
					errorFlag.close();
				}, 5000);
