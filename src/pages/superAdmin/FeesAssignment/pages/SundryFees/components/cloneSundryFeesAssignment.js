...
const UploadSchema = yup.object().shape({
	...
	PaymentPurposeId: yup.mixed().required("please select purpose of payment")
});

export const CloneSundryFeesAssignment = ({ allSessions, closeModal }) => {
	const {
		control,
		handleSubmit,
	...
	const { data: paymentPurposes } = useApiGet(getSundryPaymentPurposeUrl());

	const allPaymentPurpose = useMemo(
		() => formatSelectItems(paymentPurposes?.data, "name", "id"),
		[paymentPurposes]
	);
	const { mutate, isLoading: isPosting } = useApiPost();
	const onSubmit = (data) => {
		const { oldSessionId, newSessionId, PaymentPurposeId } = data;
		const requestDet = {
			url: cloneFeesAssignmentUrl(),
			data: {
				NewSessionId: newSessionId.value,
				PaymentPurposeId: PaymentPurposeId.value,
		...
		mutate(requestDet, {
			onSuccess: () => {
				...
				const successFlag = window.AJS.flag({
					type: "success",
					title: "Sundry Fees Action Successful!",
					body: "Your Sundry fee assignment was cloned successfully!"
				});
				setTimeout(() => {
					successFlag.close();
				}, 5000);
			...
			onError: ({ response }) => {
				const errorFlag = window.AJS.flag({
					type: "error",
					title: "Sundry Fees Acction Failed!",
					body:
						response?.data?.message ||
						`Sundry fee assignment wasn't cloned correctly!`
				});
				setTimeout(() => {
					errorFlag.close();
				}, 5000);
		...
		>
			<div className="row mb-4">
				<div className="col-lg-3 d-flex align-items-center">
					<label htmlFor="PaymentPurposeId">Payment Purpose</label>
				</div>
					...
					<Controller
						...
						render={({ field }) => (
							<SMSelect
								...
								id="PaymentPurposeId"
								{...field}
								isError={!!errors.PaymentPurposeId}
								errorText={
									errors.PaymentPurposeId &&
									errors.PaymentPurposeId.message
								}
								required
							/>
						)}
