...
import { downloadStudentUnitLoadSampleUrl } from "../../../../../../api/urls";
...
const schema = yup.object().shape({
	resultSheet: yup
		...
		.test("is-big-file", "The file you selected is too big!", (files) =>
			checkIfFilesAreTooBig(files, 10000)
		)
		.test(
			"is-correct-file",
...
export const ListUpload = ({ setUploaded, setFileData }) => {
	...
	const {
		...
		error: fileError
	} = useApiBlob(downloadStudentUnitLoadSampleUrl(), {
	...
	const downloadXLSFile = useCallback(async () => {
		...
		if (fileError || !file?.data) {
			...
			const outputFilename = `Student Unit Load Upload Template.xlsx`;
			// file file actions.
