...
export const ListPreview = ({
	...
	const { fileData, sessionId } = fileDataPayload;
	...
	const uploadList = () => {
		...
		const requestBody = {
			url: uploadPutmeResultUrl(),
			data: formData
		};
		mutate(requestBody, {
			onSuccess: () => {
				queryClient.invalidateQueries(
					getPutmeSessionApplicantCountUrl(currentFilterState)
				);
		...
		<>
			...
			>
				<div className="container-fluid px-4 mt-3 mb-3">
					<p>
						Cross check and make sure there are’nt any mistakes on
						the results before you click complete upload
					</p>
					<div>
						{extractedData?.rows ? (
							<DisplayTable
...
const DisplayTable = ({ extractedDataRow }) => {
