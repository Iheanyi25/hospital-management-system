...
import { downloadDepartmentExcelTemplateUrl } from "../../../../../../api/urls";
...
export const DepartmentUpload = ({ setUploaded, setFileData }) => {
	...
	const {
		...
		error: fileError
	} = useApiBlob(downloadDepartmentExcelTemplateUrl(), {
	...
	const downloadXLSFile = useCallback(async () => {
		...
		if (fileError || !file?.data) {
			...
			const outputFilename = `Bulk Departments Upload Template.xlsx`;
			// file file actions.
