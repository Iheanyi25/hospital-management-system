...
function AppRouter() {
    ...
    const [userType, setUserType] = useState(Boolean(localStorage.getItem('authenticatedUser')) ? JSON.parse(localStorage.getItem('authenticatedUser')).userType.toLowerCase() : null);

    useEffect(() => {
        setisAuthenticated(Boolean(JSON.parse(localStorage.getItem('authenticatedUser'))));
        if (isAuthenticated) {
            setUserType(JSON.parse(localStorage.getItem('authenticatedUser')).userType.toLowerCase());
        }
    }, [isAuthenticated]);
    ...
    const getRouteToRender = () => {
        if (isAuthenticated) {
            ...
            return <AuthRoute exact isAuthenticated={isAuthenticated} path="/" Component={Login} />
        }
    }
