...
class PowderDrugs extends React.Component {
  state = {
    powderDrugs: [],
  };
  ...
  fetchAllDrugs = async () => {
    this.setState({ powderDrugs: this.props.powderDrugs });
  };
  ...
  render() {
    const { powderDrugs } = this.state;
    return powderDrugs.length === 0 ? (
