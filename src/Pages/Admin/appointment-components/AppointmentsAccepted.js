...
import {
  getDoctorAppointmentsAcceptedUrl,
...
const AppointmentsAccepted = () => {
  const [pageNumber, setPageNumber] = useState(1);
  const [pageSize, setPageSize] = useState(50);
  const doctorAppointmentsAccepted = getDoctorAppointmentsAcceptedUrl(
    pageNumber,
    pageSize
  );
  const getPatientsAttentedToCountConfig = fetchConfig({
    url: doctorAppointmentsAccepted,
    method: "get",
  });
  ...
  if (data) {
    tableData = data.appointments.map((appointment, index) => {
      return {
        ...
        Actions: (
          <AppointmentsAcceptedActionTable
  ...
  return (
    <div>
      <Table
        content={tableData}
        tableID={"appointmentsAccepted" + data?.appointments.length}
        key={"appointmentsAccepted" + data?.appointments.length}
...
export const AppointmentsAcceptedActionTable = observer(
...
export default AppointmentsAccepted;
